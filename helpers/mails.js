...
var sendPasswordResetMail = (options) => {
    return new Promise(async (resolve) => {
        const { from, to, subject, text, token, email } = options
        ...
        sendmail({
            ...
            html: '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">' +
                ...
                '<font face="Arial, sans-serif"><b>Hi ' + email + '!</b></font>' +
                '</td>' +
                '</tr>' +
                '<tr>' +
                ...
                'Seems like you forgot your Zipcoin Exchange password. To reset it, just click the button below.<br>' +
                '<br>' +
                'Thanks,<br>' +
                'Zipcoin' +
                '</font>' +
                ...
                '<tr>' +
                '<td align="center" style="border-radius: 18px;padding:10px" bgcolor="#0062FF">' +
                '<a href=' + process.env.HOST + '/authentication/forgotpassword/?email=' + email + '&token=' + token + ' target="_blank" style="color:white!impotant;a:link, a:visited {background-color: #f44336;color: white;padding: 14px 25px;text-align: center;text-decoration: none;display: inline-block;}a:hover, a:active {background-color: red;}">' +
                'Reset Password' +
                '</a>' +
                '</td>' +
                '</tr>' +
            ...
            if (err)
                resolve({ success: false, message: MESSAGE.M_RESETFAIL })
            resolve({ success: true, message: MESSAGE.M_LINKSENT })
        })
    })
}
